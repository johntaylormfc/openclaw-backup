#!/usr/bin/env node
/**
 * Gmail OAuth via Playwright - launch browser on xvfb, navigate to auth URL,
 * catch redirect with code on localhost, exchange for tokens.
 */
...
server.listen(REDIRECT_PORT, '127.0.0.1', () => {
  console.log(`HTTP server listening on ${REDIRECT_PORT} for redirect...`);
  console.log('Launching Chrome on Xvfb...');

  const chrome = spawn('/usr/bin/xvfb-run', [
    '-a', '/usr/bin/google-chrome',
    '--no-first-run',
...
const timeout = setTimeout(() => {
  console.error('⏰ Timeout - no auth code received in 5 minutes');
  process.exit(1);
}, 300000);
...
codePromise.then(async (code) => {
  ...
  try {
    ...
    console.log('Next step: run calendar-to-todoist.js again');
  } catch (err) {
