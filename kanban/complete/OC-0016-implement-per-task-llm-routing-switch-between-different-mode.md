...
## Completion Summary
Per-agent routing implemented. arr_bc_dev now uses ollama/qwen2.5-coder:7b (local free coding model). Other agents use MiniMax. Manual override via /model alias. Routing guide written to LLM_ROUTING.md. Note: full per-task routing (auto-complexity-based) requires a cloud provider with API key (Anthropic/GPT-4) — not yet configured.
